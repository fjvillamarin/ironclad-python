...
class WorkflowAsyncLaunchSuccessResponse(BaseModel):
    """
    Response for a successfully asynchronously launched workflow.
    """ # noqa: E501
    status: StrictStr
    request_payload: WorkflowRequestModel = Field(alias="requestPayload")
    workflow_urls: WorkflowAsyncLaunchSuccessResponseWorkflowUrls = Field(alias="workflowUrls")
    workflow: LaunchedWorkflowModel
    __properties: ClassVar[List[str]] = ["status", "requestPayload", "workflowUrls", "workflow"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of workflow_urls
        if self.workflow_urls:
            _dict['workflowUrls'] = self.workflow_urls.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "requestPayload": WorkflowRequestModel.from_dict(obj["requestPayload"]) if obj.get("requestPayload") is not None else None,
            "workflowUrls": WorkflowAsyncLaunchSuccessResponseWorkflowUrls.from_dict(obj["workflowUrls"]) if obj.get("workflowUrls") is not None else None,
            "workflow": LaunchedWorkflowModel.from_dict(obj["workflow"]) if obj.get("workflow") is not None else None
        })
        return _obj
